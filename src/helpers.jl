...
function vec2array(vec::Array{Float64,1},max_period::Int)
    arr = ones(60,12) 
    arr[1,:] .= 0
    for j = 1:12
        arr[2:(max_period+1),j] = vec[((j-1)*max_period+1):j*max_period] #imposes 0 MIU in year 2005.
    end
    return arr
end
...
#########################################################################################################################################
#all below here is inherited from David/Mimi and used in parameter extraction from Nordhaus' Excel sheet
